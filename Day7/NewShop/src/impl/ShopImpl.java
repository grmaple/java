...
public class ShopImpl {
		...
		do {
			...
			do {
				...
				do {
					...
					System.out.println("1.商品类型");
					System.out.println("2.登录注册");
					...
					if(m==0) {
						...
						if(username == null) {
							...
							System.out.println("用户"+username+"您已经登录!");
							an="y";
						}
					} else if(m==3) {
						...
						if (username == null) {
							System.out.println("您还没有登录，无法退出，请先登录!");
							an="y";
						} else {
							//登录的退出登录
							System.out.println("用户"+username+"成功退出登录!");
							username = null;
							an="y";
						}
					} else {
						System.out.println("命令选择错误，请重新选择!");
						an="y";
					}
				}while(an.equals("y"));
				String ap=null;
	...
	public boolean showList() {
		...
		if (sum == 0.0) {
			System.out.println("您的购物清单为空，再去逛逛呗！");
			return true;
		}
